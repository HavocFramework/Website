...
### Syntax
A small introduction into the the yaotl language syntax.
...
The `Teamserver` block also has an sub block called `Build` where you can specify the compiler to use for compiling C and assembly files. If this block is not specified then the teamserver is going to search and use the Available mingw cross compiler (`x86_64-w64-mingw32-gcc` and `i686-w64-mingw32-gcc`). If the teamserver doesn't find the mingw/nasm compiler on the system and from the profile then its going to exit. 
